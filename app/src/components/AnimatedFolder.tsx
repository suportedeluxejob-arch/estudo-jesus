...
export function AnimatedFolder({ folderId, title, projects, className, onFolderClick, style }: AnimatedFolderProps) {
  ...
  return (
    <>
      <div
        id={folderId ? `folder-${folderId}` : undefined}
        className={cn(
          ...
          <div className="relative overflow-hidden">
            ...
            >
              {projects.map((project) => (
                <div
                  key={project.id}
                  className="w-full h-auto max-h-[70vh] bg-background flex-shrink-0 flex items-center justify-center p-8 overflow-y-auto"
                  style={{ minWidth: "100%", height: "50vh" }}
                >
                  <div className="text-left w-full h-full max-w-2xl mx-auto">
                    <h2 className="text-3xl font-bold mb-4">{project.title}</h2>
                    <div className="prose prose-invert max-w-none">
                      {project.image ? (
                        <p>{project.image}</p>
                      ) : (
                        <p className="text-muted-foreground italic">Nenhuma anotação escrita ainda. Abra o editor para começar.</p>
                      )}
                    </div>
                  </div>
                </div>
...
export const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ title, delay, isVisible, index, onClick, isSelected }, ref) => {
    ...
    return (
      ...
      >
        <div className="w-full h-full bg-muted p-2 pt-3 flex flex-col relative overflow-hidden">
           <div className="w-full h-1 bg-border rounded-full mb-1"></div>
           <div className="w-3/4 h-1 bg-border rounded-full mb-1"></div>
           <div className="w-5/6 h-1 bg-border rounded-full mb-1"></div>
           <div className="w-1/2 h-1 bg-border rounded-full mb-1"></div>
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-foreground/80 via-foreground/30 to-transparent" />
        <p className="absolute bottom-1.5 left-1.5 right-1.5 text-[10px] font-medium text-background truncate">
          {title}
        </p>
      </div>
    )
